...
Backend FastAPI d'une plateforme pédagogique auto-hébergée (Raspberry Pi) : un prof compose des cours par blocs et les partage à ses élèves via des liens publics. Le cahier des charges complet (architecture cible, enjeux, modèle de données, roadmap J0→J5) est dans **Descriptions.md** — le lire avant tout travail de fond ; il fait foi et doit être mis à jour quand une décision d'architecture change. Le projet est en plein jalon J1 (contenu) : taxonomie des matières, classification des niveaux d'étude, modèle BDD des cours (blocs, ressources, modules) et **API des cours** (`app/courses/` : CRUD cours, structure des blocs, édition du contenu des blocs texte) sont en place ; l'upload S3 presigned (blocs `ressource`) et les liens publics (J2) restent à livrer. L'utilisateur échange en français.
...
- Blocs : `POST /courses/{id}/blocks` (201 — type ∈ `texte|exercice|lien`, **`ressource` refusé par le schéma** tant que l'upload S3 n'existe pas ; `titre`/`description` optionnels posés dès la création ; contenu par défaut par type conforme à `block.py`, position = max+1) ; `PUT /courses/{id}/blocks/order` (204 — la liste doit contenir **exactement** les blocs du cours, positions réécrites 0..n-1 en un executemany Core sur la Table ; à garder déclarée avant un futur `PUT /blocks/{block_id}`) ; `PATCH /courses/{id}/blocks/{block_id}` (`BlockUpdate` — édition **partielle** : `titre`/`description` sur **tous les types**, `content` = **union de formes disjointes** `TexteContent | ExerciceContent` (smart union sans discriminant, `extra="forbid"` des deux côtés) dont la forme doit correspondre au type du bloc (422 sinon, levée **avant toute écriture**) — pour l'exercice, le service préserve les ids de questions fournis (qui doivent exister dans le bloc, 422 « Questions inconnues » sinon), génère un uuid4 `str` pour les nouvelles (un `uuid.UUID` n'est pas JSON-sérialisable par asyncpg) et applique une sémantique **remplacement** (question absente = supprimée) ; seuls les champs présents dans le payload sont appliqués via `model_fields_set`, un `titre`/`description` explicitement `null` l'efface ; au moins un champ requis (422 sinon) ; `extra="forbid"`, `content` remplacé par un **nouveau dict** car la mutation in-place du JSONB n'est pas détectée par l'ORM) ; `DELETE /courses/{id}/blocks/{block_id}` (204 — pas de renumérotation, trous tolérés).
...
- Tests `tests/test_courses_api.py` : fausse session FIFO **étendue** — les `Update` sont tracés sans consommer la file, un `Insert` porteur de RETURNING (celui de `courses`) la consomme pour servir les timestamps ; `get_or_create_by_sub` consomme toujours le premier `[user]` de la file et commite une fois.
