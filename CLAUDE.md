...
- Blocs : `POST /courses/{id}/blocks` (201 — type ∈ `texte|exercice|lien`, **`ressource` refusé par le schéma** tant que l'upload S3 n'existe pas ; contenu par défaut par type conforme à `block.py`, position = max+1) ; `PUT /courses/{id}/blocks/order` (204 — la liste doit contenir **exactement** les blocs du cours, positions réécrites 0..n-1 en un executemany Core sur la Table ; à garder déclarée avant un futur `PUT /blocks/{block_id}`) ; `PATCH /courses/{id}/blocks/{block_id}` (édition du contenu, **texte seulement** pour l'instant — enveloppe extensible `{"content": {"markdown": ≤100 000 car.}}`, `extra="forbid"`, remplacement par un **nouveau dict** car la mutation in-place du JSONB n'est pas détectée par l'ORM) ; `DELETE /courses/{id}/blocks/{block_id}` (204 — pas de renumérotation, trous tolérés).
