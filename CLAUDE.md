...
| `course_assistant/` | Assistant IA du prof : `service.py` CRUD conversations, `streaming.py` flux + reprises HITL, `turn_encoder.py` boucle SSE partagée, `context.py`/`render.py`/`replay.py`/`refs.py` helpers purs, `tools.py`, `hitl.py` registre + `suspend`, `questions.py` tool `ask_questions` (tous contextes), `editing/` descripteurs des contextes d'édition | `/courses/{id}/assistant…` |
