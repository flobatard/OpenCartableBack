...
| `core/logging.py` | `configure_logging()` (dictConfig UTC), id de corrélation (`ContextVar`) et `AccessLogMiddleware` — **seul module de config de logging** | — |
...
| `course_assistant/` | Assistant IA du prof : `service.py` CRUD conversations + pièces jointes, `queries.py` lectures partagées avec `courses`/`modules`/`resources` (clés S3 emportées par une cascade), `streaming.py` flux + reprises HITL + driver des sous-assistants (`_drive_turn`), `turn_encoder.py` boucle SSE partagée, `context.py`/`render.py`/`replay.py`/`refs.py`/`attachments.py` helpers purs, `office.py` extraction bureautique, `tools.py`, `hitl.py` registre + `suspend`, `questions.py` tool `ask_questions` (tous contextes), `delegation.py` tools `edit_block`/`edit_module` de l'édition globale, `structure.py` ses propositions structurelles (`propose_block_add`/`propose_block_delete`/`propose_blocks_reorder`), `editing/` descripteurs des contextes d'édition | `/courses/{id}/assistant…` |
...
**Journalisation** (`core/logging.py`, stdlib seule — pas de structlog)
- `configure_logging()` est appelée **au niveau module** de `app/main.py` (et par les deux `__main__` de maintenance) : jamais depuis `create_app()`, où `dictConfig` arracherait le handler que pytest pose sur la racine pour `caplog`. Idempotente ; `disable_existing_loggers: False` (les routeurs sont importés avant l'appel).
- Racine à **`WARNING`**, seul `app` suit `LOG_LEVEL` (sinon `httpx` journalise chaque appel sortant) ; **`uvicorn.access` volontairement muet** (sinon deux lignes par requête). Un `LOG_LEVEL` invalide retombe sur `INFO`, jamais une erreur au boot.
- `AccessLogMiddleware` est **ASGI pur** (durée du flux SSE entier, `ContextVar` visible du threadpool) et monté **en dernier** dans `create_app`, donc le plus externe. Logger `app.access`, `/health` en DEBUG, `text/event-stream` hors seuil de lenteur.
- Jamais d'en-tête lu hors `x-request-id` (« jamais le token dans les logs ») ; l'id entrant est validé avant d'être réfléchi et journalisé, la cible est assainie (`scope["path"]` est percent-décodé) et les valeurs de query sensibles masquées.
...
**S3** : bucket privé, l'API ne sert que des URL présignées ; `presign_*` = calcul local (sync OK), HEAD/delete/listing = réseau sous threadpool. **Toute table qui écrit une clé S3 doit entrer dans les DEUX contrôles** (`resources`, `users.avatar_s3_key`, `ai_attachments`) : l'anti-jointure de `reconcile_s3_orphans` — une table oubliée voit ses objets déclarés orphelins et supprimés, en silence tant que `PURGE_S3_ORPHANS_DRY_RUN` est vrai — et les tranches à curseur de `missing_s3_objects` (`_CURSORED_TABLES`, budget partagé, un curseur par table). **Et toute suppression dont la cascade emporte des lignes à clé S3 doit collecter ces clés AVANT son delete** : une conversation part avec sa cible d'édition (FK `CASCADE`), donc supprimer un bloc, un module ou une ressource emporte des pièces jointes — helpers de `course_assistant/queries.py`, purge après commit. Purge S3 **après** commit (un échec laisse un orphelin ramassé par la réconciliation, jamais une référence vers un objet absent) ; l'import fait l'inverse (put S3 avant commit) pour la même raison. Seuls `read_object_into`/`put_object` transportent des octets (export/import).
...
- Structure (même régime `allow_edit`, `structure.py`) : ajout, suppression et réordonnancement de blocs sont des **propositions** de l'assistant global lui-même (genre `proposal`, jamais un sous-assistant ni un `agent`), reprises par la route de décision qu'`allow_edit` retenu au registre autorise hors contexte d'édition ; le back **n'écrit rien** (le front applique par les routes du cours) ; un ajout ne porte que la méta (bloc créé vide, rempli par `edit_block`) ; les `B…` restent **positionnelles** — renumérotées à la reprise, le résultat du tool rend le nouveau sommaire ; `block_refs` capturés à l'interrupt ne sont **jamais rejoués**, seulement comparés (`new_block_refs`, `stale_blocks`) ; le tool est ré-exécuté avec les args **d'origine** : une suppression teste `block_gone` AVANT toute résolution, et un texte d'acceptation ne cite jamais l'entrée résolue.
- Le modèle ne manipule que des **références courtes** (`B1`/`R1`/`M1`/`Q1`/`A1`), réécrites en UUID en flux ; aucun accès DB pendant l'exécution des tools (instantané du tour).
- Pièces jointes (`attachments.py`, `office.py`, tool `read_attachment`) : table **dédiée** `ai_attachments`, jamais une `resource` — le partage public expose toute la bibliothèque, pas les fichiers de travail du prof. Whitelist de mimes **fermée**, plafond **par famille**, confirmation sur le gabarit d'`confirm_avatar` (HEAD : taille **et** `ContentType`, purge de l'objet hors gabarit). Les `A…` sont **stables à vie** dans une conversation (liste croissante triée `created_at, id`, pièce envoyée **indélébile** — 409) : rien à capturer au registre HITL, mais `_sse_resume` **doit** les recharger (mêmes tools qu'à l'aller) et l'instantané du flux les porte (4-uplet) pour les sous-assistants. Le contenu n'entre **jamais** dans le contexte du tour : seulement nom, famille, taille et référence.
...
**Configuration** : `settings = get_settings()` s'évalue **à l'import** ; trois réglages de log (`LOG_LEVEL` INFO, `LOG_ACCESS` true, `LOG_SLOW_REQUEST_MS` 1000) documentés en commentaires dans les `config/*.yaml` — dans les tests, les variables d'env sont posées en tête de `tests/conftest.py` avant tout import de `app.*`. Tous les délais **et toutes les cadences** de maintenance vivent dans `config/*.yaml`, jamais dans `.env` ; le défaut d'une cadence vit sur le champ `Settings`, jamais recopié dans le registre.
...
- Logs : stdlib `logging` seule — pas de structlog, d'`asgi-correlation-id` ni d'OpenTelemetry (ARM64, et rien de ce qu'ils apportent ne sert ici) ; pas de sortie JSON tant qu'aucun agrégateur n'est en face (les `extra=` de l'access-log sont prêts). `uvicorn.access` muet et `/health` en DEBUG sont voulus.
