...
**S3** : bucket privé, l'API ne sert que des URL présignées ; `presign_*` = calcul local (sync OK), HEAD/delete/listing = réseau sous threadpool. **Toute table qui écrit une clé S3 doit entrer dans l'anti-jointure de `reconcile_s3_orphans`** (`resources`, `users.avatar_s3_key`, `ai_attachments`) : une table oubliée voit ses objets déclarés orphelins et supprimés — en silence tant que `PURGE_S3_ORPHANS_DRY_RUN` est vrai. Purge S3 **après** commit (un échec laisse un orphelin ramassé par la réconciliation, jamais une référence vers un objet absent) ; l'import fait l'inverse (put S3 avant commit) pour la même raison. Seuls `read_object_into`/`put_object` transportent des octets (export/import).
