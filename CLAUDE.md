...
| `course_assistant/` | Assistant IA du prof : `service.py` CRUD conversations, `streaming.py` flux + reprises HITL + driver des sous-assistants (`_drive_turn`), `turn_encoder.py` boucle SSE partagée, `context.py`/`render.py`/`replay.py`/`refs.py` helpers purs, `tools.py`, `hitl.py` registre + `suspend`, `questions.py` tool `ask_questions` (tous contextes), `delegation.py` tools `edit_block`/`edit_module` de l'édition globale, `structure.py` ses propositions structurelles (`propose_block_add`/`propose_block_delete`/`propose_blocks_reorder`), `editing/` descripteurs des contextes d'édition | `/courses/{id}/assistant…` |
