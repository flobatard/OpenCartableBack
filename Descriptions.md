...
- **Second contexte livré : l'édition d'un bloc texte (`block_text`), premier flux HITL** — conversations rattachées à un bloc (`block_id`, liste filtrée `?context=&block_id=`, création validée : bloc du cours, type `text`), system prompt d'édition dédié (le bloc édité toujours rendu **en entier** dans le contexte du tour, le reste du cours en sommaire ; **catalogue des syntaxes du markdown de cours** — KaTeX (chimie mhchem comprise), Mermaid, TikZ, GeoGebra, JSXGraph, frises chronologiques, molécules SMILES, liens `oc-resource:`/`oc-module:` — pour que le modèle dispose de tous les éléments d'édition ; l'existant est préservé verbatim, une nouvelle insertion passe par la référence courte, réécrite en UUID par le serveur), tool **`propose_block_edit`** (markdown complet de remplacement + résumé, plafond 100 000 caractères) exposé dans ce seul contexte ; côté front, dès la proposition, le **diff Monaco côte à côte s'affiche à la place de l'éditeur** avec un champ commentaire — le prof **accepte** (le markdown entre dans l'éditeur, l'autosave persiste) ou **rejette**, et sa décision commentée devient le **résultat du tool** que le modèle lit pour poursuivre. Contrat SSE : un seul événement ajouté, **`interrupt`** (le flux se ferme à la proposition, la décision rouvre le flux de la suite du tour — pas d'attente silencieuse sur connexion ouverte).
