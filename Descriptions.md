...
| **J5 — IA** *(5 briques livrées)* | **Assistant de cours** : chat par cours à conversations persistées, agent LangGraph + tools (lecture de blocs, extraction PDF, images, modules), citations de sources, **contextes d'édition à flux HITL** — bloc texte (tool `propose_block_edit`, diff + application côté front), exercice (quatre tools par question, revue structurée) et module (un tool par fichier) — cf. §5.7 ; **tuteur d'exercice de l'élève authentifié** (`app/student_exercises/` : tentatives persistées par tour, verdict structuré, révélation du corrigé bornée serveur) ; restent le chat élève anonyme, la vue prof des soumissions, la vectorisation (ChromaDB, si actée) et le RAG | Interroger, critiquer et synthétiser un cours avec l'IA |
