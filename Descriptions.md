...
- **Organisation** : clé préfixée par cours (`courses/<course_id>/resources/<resource_id>/<nom-sanitizé>`) — purge par préfixe simple à la suppression d'un cours — la hiérarchie logique (matière → cours → ressource) restant portée par la **base**, pas par les préfixes S3. Les **pièces jointes des chats de l'assistant** (§5.7) suivent le même préfixe de cours, sous `courses/<course_id>/assistant/<attachment_id>/<nom-sanitizé>` : sous `courses/` délibérément, pour que la réconciliation des orphelins et l'inventaire de volumétrie les couvrent sans modification.
- **Cohérence DB↔S3** : la ligne `resource` est créée *avant* l'upload avec `status='pending'` ; un endpoint de confirmation vérifie l'objet (HEAD S3) et passe le statut à `'available'`. Seules les ressources disponibles sont servies. Le **troisième flux presigné** (pièces jointes de l'assistant) est le plus strict des trois : whitelist de mimes fermée, plafond par famille, et confirmation qui vérifie au HEAD **la taille et le `ContentType`** avant de purger l'objet hors gabarit — une URL présignée PUT ne borne ni l'un ni l'autre.
- **Ce que le partage expose** : la bibliothèque d'un cours partagé est visible de tous ses élèves (décision 7). Les pièces jointes des chats de l'assistant vivent donc dans une **table séparée** (`ai_attachments`), jamais servie par le régime public : ce sont les fichiers de travail du professeur, pas du contenu de cours.
...
- Le **texte de cours** (bloc `text`) est du **markdown simple stocké dans le JSONB du bloc** (`{"markdown": ...}`) — pas de HTML brut : plus sûr, réindexable, directement exploitable par l'IA. Titres, paragraphes, encadrés (syntaxe des alertes GitHub, `> [!DEFINITION]`…), mise en colonnes (`::: columns` … `+++` … `:::`, ratio facultatif `1:2`/`2:1`) **et liens externes** sont couverts par le markdown, sans types de blocs dédiés (l'ancien type `lien` a été supprimé).
...
- **Librairies préinstallées** : un module peut déclarer, par un pragma en commentaire en tête de son JS (`// @oc-libs: matter, chart`), des librairies **embarquées par l'app** — Matter.js, Chart.js, p5.js 2.x, JSXGraph, D3, Three.js. Le front lit leur texte sur sa propre origine (`/assets/module-libs/`) et l'**inline** dans le `srcdoc` avant le JS du prof : la CSP n'est pas rouverte, l'iframe ne fait toujours aucune requête. Le catalogue est recopié dans le prompt `MODULE_RUNTIME` de l'assistant.
...
- **Brique livrée (hors jalon) : client IA générique multi-provider en « Bring Your Own Token »** (`app/core/ai/`, sur LangChain 1.x). La config (provider, clé API, modèle, base_url, et depuis la décision 24 les préférences de raisonnement `reasoning`/`reasoning_effort`, portables, gatées par provider et encodées par `app/core/ai/providers.py`) voyage à chaque appel, avec en cascade (décision actée, révise le « l'API ne stocke aucune clé » initial) : config explicite de la requête > **configuration utilisateur persistante chiffrée** (`app/ai_credentials/` : depuis la décision 25, **plusieurs configurations nommées** par utilisateur dans la table `ai_configurations`, au plus une **active** — index partiel unique —, aucune active = IA par défaut ; clé API chiffrée par `app/core/crypto.py` — AES-256-GCM, dérivation HKDF depuis la clé maître serveur `AI_CREDENTIALS_MASTER_KEY` du `.env` + un sel par ligne ; la clé n'est jamais ré-émise par l'API. Portée : un dump DB seul ou le seul `.env` sont inexploitables, mais un serveur compromis peut déchiffrer — inhérent, l'API doit lire la clé pour appeler le provider) > fallback serveur optionnel (`AI_*`). Providers : Anthropic, OpenAI, Google Gemini, Mistral, Ollama (local ou distant), HuggingFace (endpoints distants uniquement — jamais de modèle chargé en local sur le Pi) et tout endpoint `openai_compatible` (Groq, vLLM, LM Studio…). Deux modes : appel classique et streaming (contrat SSE de référence, route de smoke-test `/api/v1/ai/chat[.../stream]`). Observabilité **Langfuse opt-in** (settings `LANGFUSE_*`, no-op sinon). Le **multi-provider concrétise la contrainte RGPD** ci-dessous : Mistral (hébergé UE) et Ollama (local) sont des providers de première classe.
- **Brique J5 livrée : l'assistant IA de cours (contexte global)** — décision actée qui précise le cadrage initial (qui ne prévoyait que RAG/résumés/quiz) : l'IA prend la forme d'un **chat par cours à conversations persistées**, décliné en **5 contextes** partageant une base commune (tools d'exploration du cours, persistance par contexte) — global cours (livré : critique, exploration, synthèse), **édition d'un bloc texte et édition d'un exercice (livrés, cf. ci-dessous)**, édition d'un module, et résolution d'exercice élève (ce dernier **sans persistance**, fonctionnement par question) ; chaque contexte aura ses flux **HITL**. Implémentation : tables `ai_conversations`/`ai_messages` (tours d'outils persistés et rejoués), boucle agent **LangGraph** (`create_agent` de langchain 1.x, confinée dans `app/core/ai/`) avec tools `read_block`/`read_resource_pdf` (extraction **pypdf** plafonnée)/`read_resource_image` (image de la bibliothèque montrée au modèle, jointe en message utilisateur — modèle à vision requis)/`read_module` (code HTML/CSS/JS d'un module), SSE élargi (`thinking`, `tool_call`, `tool_result` avec extrait borné du résultat), **citations de sources** `oc-block:`/`oc-resource:` validées côté serveur. Réservé au **prof authentifié** — le chat élève anonyme est reporté (question ouverte : imputation du quota). **Arbitrage HITL acté (révisé au premier lot d'édition) : interrupt/resume LangGraph, checkpointer `InMemorySaver`** — quand l'agent propose (tool qui **ne mute rien**, proposition dans les args du `tool_call`, persistés), le run est **figé** (interrupt LangGraph) : le flux SSE se ferme, et la route de décision **rouvre un flux qui reprend le run** (`Command(resume=…)`) — le **résultat du tool est la décision** (acceptée/rejetée + commentaire). Le checkpointer est **InMemory** (process-local : mono-worker assumé, reprises perdues au redémarrage — cohérent avec le registre de reprises in-process) ; **passage à `AsyncPostgresSaver` acté pour le jour du multi-nœud** (TODO.md). L'application, elle, reste côté front (routes d'édition existantes).
- **Second contexte livré : l'édition d'un bloc texte (`block_text`), premier flux HITL** — conversations rattachées à un bloc (`block_id`, liste filtrée `?context=&block_id=`, création validée : bloc du cours, type `text`), system prompt d'édition dédié (le bloc édité toujours rendu **en entier** dans le contexte du tour, le reste du cours en sommaire ; **catalogue des syntaxes du markdown de cours** — encadrés, colonnes, KaTeX (chimie mhchem comprise), Mermaid, TikZ, GeoGebra, JSXGraph, frises chronologiques, textes à lignes numérotées, molécules SMILES, graphiques Vega-Lite, partitions ABC, requêtes SQL et programmes Python exécutables, liens `oc-resource:`/`oc-module:` — pour que le modèle dispose de tous les éléments d'édition ; l'existant est préservé verbatim, une nouvelle insertion passe par la référence courte, réécrite en UUID par le serveur), tool **`propose_block_edit`** (markdown complet de remplacement + résumé, plafond 100 000 caractères) exposé dans ce seul contexte ; côté front, dès la proposition, le **diff Monaco côte à côte s'affiche à la place de l'éditeur** avec un champ commentaire — le prof **accepte** (le markdown entre dans l'éditeur, l'autosave persiste) ou **rejette**, et sa décision commentée devient le **résultat du tool** que le modèle lit pour poursuivre. Contrat SSE : un seul événement ajouté, **`interrupt`** (le flux se ferme à la proposition, la décision rouvre le flux de la suite du tour — pas d'attente silencieuse sur connexion ouverte).
- **Troisième contexte livré : l'édition d'un exercice (`block_exercise`), HITL par question** — décision actée (arbitrage utilisateur) : la proposition est **unitaire**, jamais un remplacement de l'exercice entier — quatre tools de proposition, une opération par appel, une proposition à la fois (le modèle enchaîne les appels après chaque décision) : `propose_statement_edit` (sujet), `propose_question_edit` (énoncé et/ou corrigé d'une question existante), `propose_question_add` (nouvelle question, position `after_ref`, fin par défaut), `propose_question_delete`. Les questions du bloc édité portent des **références courtes `Q1…`** (même principe que B/R/M : le modèle ne manipule jamais l'id d'une question), **stables le temps d'un tour, reprises comprises** — la numérotation est capturée à l'interrupt et rejouée à la reprise : une question supprimée libère sa référence sans réattribution, une question ajoutée reçoit la suivante (le résultat du tool invite à relire le bloc) ; à l'émission, le serveur ajoute l'id résolu (`question_id`/`after_id`) aux args du `tool_call`. Les contextes d'édition sont décrits par un **descripteur** (`app/course_assistant/editing/` : type de bloc attendu, system prompt, tools de proposition avec validation, gate d'interrupt et réécriture des args) — service et streaming n'ont plus aucune branche par contexte, ajouter un contexte = un module + une entrée au registre. Côté front, une **revue structurée** remplace l'éditeur d'exercice (une carte par opération : diff Monaco du sujet ou de l'énoncé/corrigé, aperçu rendu d'une question ajoutée, avertissement de suppression) ; l'acceptation applique l'opération dans le formulaire de l'éditeur (énoncés via Monaco, annulables par Ctrl-Z ; corrigé, ajout et suppression via le formulaire), l'autosave persiste — et le front **flush l'autosave avant chaque décision** pour que la reprise du run relise l'exercice à jour.
- **Questions de l'assistant au professeur, dans tous les contextes (décision 34)** — face à une demande ambiguë, le tool bloquant **`ask_questions`** pose 1 à 4 questions à choix unique ou multiple (2 à 6 suggestions validées ; le choix libre « Autre » est ajouté par le front, jamais par le modèle) ; même mécanique interrupt/resume que les propositions, mais un **genre** porté par l'interrupt (`kind`, champ SSE additif) et par le registre de reprises, et une route dédiée `POST …/questions/{tool_call_id}/answer` (réponses par index de suggestion + réponse libre, ou refus) contrôlée contre la forme attendue **avant** de consommer la reprise (422 réessayable). Le résultat du tool est la réponse du professeur ; sur refus, le modèle poursuit avec des hypothèses signalées. Conséquences : **tout tour d'assistant est checkpointé** (état écrit à la sortie du graphe, thread purgé à la fin, à l'abandon du flux, à la suppression et à l'expiration) et une **garde du graphe agent** ne retient qu'un tool bloquant par réponse du modèle (les autres sont retirés de la réponse avant l'état : jamais relayés, exécutés ni revus par le modèle). Le tuteur élève n'a pas l'outil.
- **Édition globale par délégation (décision 36)** — l'assistant du contexte `course` reste un lecteur, mais quand le professeur active l'édition globale (`allow_edit` du tour, préférence du navigateur envoyée à chaque message), il dispose des tools bloquants **`edit_block`/`edit_module`** qui confient une cible et des consignes autonomes à un **sous-assistant d'édition** : un run bâti sur le descripteur existant de la cible, sur son propre thread, orchestré **hors du graphe** par le driver du flux SSE (interrupt de genre `delegation` jamais relayé ni enregistré, sous-assistant lancé dans le même flux, événements tagués `agent`, compte rendu en valeur de reprise du parent — résultat du tool). Ses propositions et questions suivent le flux HITL ordinaire (registre à `delegation` : deux threads, purgés ensemble ; reprise de SON run par les routes de décision et de réponse) ; un sous-assistant à la fois, dix par tour au plus ; sa transcription n'est pas persistée (seuls l'appel `edit_*`, args réécrits, et son compte rendu le sont). L'application reste côté front — sur la cible, via l'éditeur monté ou par un PATCH headless.
- **Structure du cours (décision 40)** — dans le même régime d'édition globale, l'assistant du contexte `course` propose lui-même, sans sous-assistant, d'**ajouter** un bloc (`propose_block_add` : méta seule — type, titre, description, position, ressource ou module pointé ; un bloc texte ou exercice naît vide, rempli ensuite par `edit_block`), d'en **supprimer** un (`propose_block_delete`) ou de les **réordonner** (`propose_blocks_reorder`, ordre complet). Ce sont des propositions HITL ordinaires (genre `proposal`, args réécrits en ids, reprise par la route de décision qu'`allow_edit` autorise hors contexte d'édition) ; le back n'écrit rien, le front applique par les routes du cours. Les références `B…` restent positionnelles : la reprise les renumérote sur l'instantané rechargé et le résultat du tool rend le nouveau sommaire (numérotation d'origine capturée à l'interrupt pour nommer le bloc créé et tolérer le bloc supprimé).
- **Pièces jointes des chats (décision 42)** — le professeur joint des fichiers à ses messages, dans le chat global comme dans les chats d'édition (le tuteur d'exercice de l'élève est hors périmètre). Table **dédiée** `ai_attachments` (métadonnées seules, binaire sur S3 sous `courses/<course_id>/assistant/…`, `conversation_id` et `message_id` nullables jusqu'à l'envoi — on peut joindre un fichier à une conversation encore à l'état de brouillon), **jamais** une `resource` : le partage d'un cours exposerait le fichier à tous les élèves. Formats acceptés par whitelist fermée avec plafond par famille : images PNG/JPEG/WebP, PDF, texte (txt/md/csv), bureautique (docx/pptx/odt/odp). Le modèle **ne reçoit pas** le fichier : le contexte du tour l'annonce en référence courte (`A1`, `A2`… — nom, famille, taille, et la mention « jointe à CE message ») et un tool **`read_attachment`** le sert à la demande, en dispatchant sur la famille — image montrée au modèle (vision requise, tour courant seulement), PDF par pypdf, texte brut, bureautique par `python-docx`/`python-pptx`/`odfpy` (extraction **dégradée et annoncée comme telle** : mise en forme, structure des tableaux et images perdues). Les `A…` sont stables à vie dans une conversation, une pièce envoyée étant indélébile : c'est ce qui permet à une reprise HITL d'exposer exactement les mêmes tools qu'à l'aller. Une pièce jamais envoyée est purgée par un job de maintenance (rétention 7 jours).
- La **vectorisation des cours n'est pas actée**. Si elle se fait, elle passera probablement par **ChromaDB** (dépendances déjà présentes dans le backend) — aucune préparation en base Postgres n'est requise d'ici là. L'assistant s'en passe pour l'instant : le contexte ne porte qu'un **sommaire structuré** du cours (jamais le contenu d'un bloc), le modèle lit les blocs utiles à la demande (tool `read_block`) — décision 23 du journal.
- Prévoir, sans l'implémenter au MVP : un pipeline *extraction de texte (PDF/images) → découpage → embeddings → stockage vectoriel*, déclenché en tâche de fond à l'upload (l'extraction PDF à la demande existe déjà via le tool `read_resource_pdf`).
- Le choix Python/FastAPI rend naturelle l'intégration ultérieure (RAG, génération de quiz/résumés) : elle consommera le client `app/core/ai/`. Pour le fournisseur de modèle, garder en tête la contrainte RGPD (option hébergée UE type Mistral, ou modèle local selon ressources) — le **tool-calling multi-provider** est acquis pour Mistral, dépendant du modèle chez Ollama, non garanti HuggingFace.
...
| **J5 — IA** *(5 briques livrées)* | **Assistant de cours** : chat par cours à conversations persistées, agent LangGraph + tools (lecture de blocs, extraction PDF, images, modules), citations de sources, **contextes d'édition à flux HITL** — bloc texte (tool `propose_block_edit`, diff + application côté front), exercice (quatre tools par question, revue structurée) et module (un tool par fichier), et **questions de l'assistant au professeur** dans tous les contextes (`ask_questions`) — cf. §5.7 ; **tuteur d'exercice de l'élève authentifié** (`app/student_exercises/` : tentatives persistées par tour, verdict structuré, révélation du corrigé bornée serveur) ; restent le chat élève anonyme, la vue prof des soumissions, la vectorisation (ChromaDB, si actée) et le RAG | Interroger, critiquer et synthétiser un cours avec l'IA |

Livré hors jalon, en extension du socle J0 : comptes applicatifs (`users`, auto-provisioning par `sub`) et onboarding bloquant (rôles cumulables prof/élève, système scolaire, niveaux, matières) — cf. §2, §5.1 et §6. Également livré hors jalon : l'**export/import de cours** (`app/course_transfer/`) — archive `.zip` (manifest versionné + binaires des ressources) dont le réimport recrée un cours au contenu identique (nouveaux identifiants, références remappées, classement porté par les `code` de taxonomie donc portable entre instances) — cf. §5.2 pour l'exception actée au transit des binaires. Enfin, un **cours d'exemple** en brouillon (`app/starter_course/`, manifeste du même format embarqué, sans binaire) est déposé à la première complétion d'un profil de prof, pour lui montrer les possibilités d'écriture sans qu'il ait à lire la documentation ; il se recharge à la demande par `POST /courses/starter`.
