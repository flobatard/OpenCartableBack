...
| **J5 — IA** *(3 briques livrées)* | **Assistant de cours** : chat par cours à conversations persistées, agent LangGraph + tools (lecture de blocs, extraction PDF), citations de sources, **contextes d'édition à flux HITL** — bloc texte (tool `propose_block_edit`, diff + application côté front) et exercice (quatre tools par question, revue structurée) — cf. §5.7 ; restent le contexte module et la résolution d'exercice, le chat élève, la vectorisation (ChromaDB, si actée) et le RAG | Interroger, critiquer et synthétiser un cours avec l'IA |
