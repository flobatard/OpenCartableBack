...
| **J5 — IA** *(2 briques livrées)* | **Assistant de cours** : chat par cours à conversations persistées, agent LangGraph + tools (lecture de blocs, extraction PDF), citations de sources, **contexte d'édition de bloc texte à flux HITL stateless** (tool `propose_block_edit`, diff + application côté front) — cf. §5.7 ; restent les contextes exercice/module et la résolution d'exercice, le chat élève, la vectorisation (ChromaDB, si actée) et le RAG | Interroger, critiquer et synthétiser un cours avec l'IA |
