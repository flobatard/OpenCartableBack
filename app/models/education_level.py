...
class EducationLevel(Base):
    ...
    __table_args__ = (
        ...
        # racines (parent_id NULL) ; ``system`` en tête pour qu'un futur
        # arbre étranger puisse réutiliser un nom de racine (ex. « Primaire »).
        UniqueConstraint(
            "system",
            "parent_id",
            "name",
            name="uq_education_levels_system_parent_id_name",
            postgresql_nulls_not_distinct=True,
        ),
        CheckConstraint(
            f"depth >= 0 AND depth <= {MAX_DEPTH}",
            name="ck_education_levels_depth",
        ),
        CheckConstraint("parent_id != id", name="ck_education_levels_not_own_parent"),
        CheckConstraint(
