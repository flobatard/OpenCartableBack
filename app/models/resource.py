...
un futur endpoint de confirmation vérifiera l'objet (HEAD S3) et passera le
statut à ``'disponible'`` — c'est le mécanisme de cohérence DB↔S3.

Une ressource de ``type='module'`` (bundle HTML/JS auto-porté) est complétée
par une ligne ``modules`` (cf. :mod:`app.models.module`).
"""
